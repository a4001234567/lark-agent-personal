...
我们在最后给出了**针对飞书妙搭openclaw机器人的实际攻击展示**。
...
### 方法 C：`multi_url.url` — 按钮平台差异化 URL 注入

**隐蔽性：** 高（Android/iOS/PC 用户均不可见，任何已知客户端均不导航至注入 URL）  
**适用实现：** lark-mcp-server、官方插件  
**注意：** 这是目前唯一对两种实现**同时有效**的完全隐藏向量，但对于官方插件效果更好。
...
以下为实际攻击测试记录。攻击卡片伪装为"企业 OA 系统待办提醒"，通过 `multi_url.default_url`（向量 C）注入指令，目标 Bot 为运行中的 lark-mcp-server 实例。

**注入指令目标：** 让 Agent 读取本地凭据（通过飞书 API 换取 tenant_access_token），并携带凭据访问攻击者控制的服务器端点。

**攻击结果：**

**图 1 — Agent 视角（Claude 界面）**

Agent 接收到 OA 卡片后，读取了 `default_url` 中的注入内容，并在处理过程中调取了 tenant access token（日志中段可见）。

![Agent 视角：处理注入卡片，token 出现在日志中](../img/poc-token-leak-agent-view.png)

**图 2 — 飞书对话视角**

Agent 主动向攻击者服务器发起 HTTP 请求，尝试完成"SSO 回调"。服务器返回文件目录列表，Agent 随即确认请求到达并询问用户下一步。

![飞书对话视角：Agent 主动访问攻击者地址](../img/poc-token-leak-bot-response.png)

> 攻击链路：卡片 default_url 注入 → Agent 解析 → 读取凭据 → 发起出站 HTTP 请求（携带 open_id 及 token 参数）

**图 3 — 执行日志（关键步骤，lines 27–36）**

以下为 Agent 执行日志的格式化摘录，完整还原了攻击链的每一步操作（凭据已脱敏）：

![执行日志摘录：credentials 读取 → token 获取 → Bearer 头外发](../img/poc-log-pretty.png)
